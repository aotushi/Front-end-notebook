...
- 触发条件:
  ...
  - `display: inline-block / table`等等
  - `float` 元素(不包含none)
  - `ovevflow` !== `visible`
  - 弹性元素flex的直接子元素/网格元素的直接子元素
- 规则:
...
解决方案:
 1.父元素设置浮动 float:left;
 2.父元素设置上边框: border-top:1px solid red;
 3.父元素设置overflow:hidden;
 4.子元素设置浮动: 两个子元素都需要设置浮动
...
4. **作用域链**
   ...
   - **查找变量**的时候先在**当前作用域**的**变量对象**中查找，如果有就使用，如果没有会继续去**上级作用域**查找，直到找到**全局作用域**，如果还没有就报错，报错内容： xxx is not defined, 查找的过程就是沿着作用域链查找
...
- **什么是闭包**
  ...
  3. 该容器用于收集内部函数使用的外部函数的变量
  4. 该容器在内部函数调用的时候供内部函数使用
...
​		保存待执行的回调函数的事件队列 事件队列(Event queue)/任务队列(Task queue)
...
函数会先返回一个表示异步读取操作的Promise对象，未来对这个对象的操作完全取决于Promise的生命周期

每个Promise都会经历一个短暂的生命周期：先是处于进行中（pending）的状态，此时操作尚未完成，所以它也是未处理（unsettled）的；一旦异步操作执行结束，Promise则变为已处理（settled）的状态。

操作结束后，Promise可能会进入到以下两个状态中的其中一个：
Fulfilled　Promise异步操作成功完成。
Rejected　由于程序错误或一些其他原因，Promise异步操作未能成功完成

只有当Promise的状态改变时，通过then()方法来采取特定的行动。then()的两个参数都是可选的，所以可以按照任意组合的方式来监听Promise，执行完成或被拒绝都会被响应

Promise还有一个catch()方法，相当于只给其传入拒绝处理程序的then()方法

then()方法和catch()方法一起使用才能更好地处理异步操作结果
如果不给Promise添加拒绝处理程序，那所有失败就自动被忽略了，所以一定要添加拒绝处理程序，即使只在函数内部记录失败的结果也行。

每次调用then()方法或catch()方法都会创建一个新任务，当Promise被解决（resolved）时执行。这些任务最终会被加入到一个为Promise量身定制的独立队列中

调用resolve()后会触发一个异步操作，传入then()和catch()方法的函数会被添加到任务队列中并异步执行。 完成处理程序(vlaue=>{})和拒绝处理程序(reason=>{})总是在执行器完成后被添加到任务队列的末尾。
```
...
1.执行器函数executor: (resolve, reject)=>{}
2.执行器函数是同步的
new Promise(()=>setTimeout(console.log,0,'aaa'));
setTimeout(console.log,0,'bbb')
//log结果的顺序: 定时器id, aaa, bbb

//执行器错误
如果执行器内部抛出一个错误，则Promise的拒绝处理程序就会被调用，例如：
let promise = new Promise(function(resolve,reject){
    throw new Error('error');
    //console.log(a)  未声明的变量
})
promise.catch(function(error){console.log(error.message)}); //'error'
在这段代码中，执行器故意抛出了一个错误，每个执行器中都隐含一个try-catch块，所以错误会被捕获并传入拒绝处理程序. 等价于:
let promise = new Promise(function(resolve,reject){
    try{
        throw new Error('error');
    }catch(error){
...
```js
只有当Promise的状态改变时，通过then()方法来采取特定的行动。
...
此示例中，Thenable对象和Promise之间只有then()方法这一个相似之处，可以调用Promise.resolve()方法将Thenable对象转换成一个已完成Promise：
let thenable = {
    then:function(resolve,reject){
        resolve(42);
    }
...
有了Promise.resolve()方法和Promise.reject()方法，我们可以更轻松地处理非Promise的Thenable对象。在ECMAScript 6引入Promise对象之前，许多库都使用了Thenable对象，所以如果要向后兼容之前已有的库，则将Thenable对象转换为正式Promise的能力就显得至关重要了。
...
2.vue3的响应式
vue3的响应式采用的是ES6 Proxy实现的。且在响应式数据中的如数组或者对象手动添加元素也是响应式的
```
...
```js
//https://blog.csdn.net/weixin_39357177/article/details/104425275

单页面spa:
服务端返回一个空的需要挂载的html节点，在没有 静态资源缓存情况下，首次加载速度较慢，因为需要经历静态资源下载、js 完全执行、异步数据返回才能开始渲染页面，不利于SEO。


服务端渲染ssr
用户访问 url， node 拦截到路由，并根据路由渲染对应单页面组件生成 dom 字符串，如果有数据请求，同步获取到数据后才渲染出 dom 字符串，插入到 html 中返回客户端，客户端一开始看到的就是一个有完整数据的页面，至此服务端渲染完成。
```
...
```js
1.运行环境
H5 的宿主环境是浏览器;小程序内无法使用window对象和document对象,就是没有dom和bom相关的api
2.运行机制
H5 的运行就是一个网页的运行; 小程序的热启动和冷启动,销毁
3.系统权限
h5系统权限不够，比如网络通信状态、数据缓存能力、通讯录、或调用硬件的，
微信客户端的这些系统级权限都可以和微信小程序无缝衔接
4.开发语言
H5 HTML、CSS、JavaScript;
小程序都有各自定义的语言
5.开发成本(开发语言引出来的)
h5:开发工具-开发框架-模块化工具-ui库
小程序:不用考虑浏览器兼容性. 小程序成本低
6.更新
h5:更新后见结果;
小程序:审核
7.渲染机制
...
```js
//https://blog.csdn.net/DLGDark/article/details/78513685
...
```js
Object.definedProperty  消息订阅和发布
...
//未添加到暂存区的修改
git status查看modified的文件filename
git checkout -- filename来撤销修改

其他: git checkout --.  //将多个文件一次性撤销
...
git revert id记录
然后在推送到远端更新远程仓库代码，修改的文件就撤销回来了。注意的是revert奇数次生效，偶数次又回到之前的修改状态
还有就是如果想回到之前某个版本，可以用reset命令，可以回退到某次提交，那该提交之后的提交都会回滚，不过这种覆盖是不可逆的，之前的提交记录都没有了。
--hard – 强制将缓存区和工作目录都同步到你指定的提交
...
//https://blog.csdn.net/weixin_43932245/article/details/88825953
//简单操作
1.安装 npm install xxx, npm i xxx
2.删除 npm uninstall xxx, npm un xxx
3.运行 node xxx.js
4.引入 require('xxx')
5.导出 
...
//创建模块
expots.world = function(){console.log('hello world')};
或
function hello(){...}
...
```js
//https://www.jianshu.com/p/e3decab777d8
...
3.匿名函数中的this都是window
4.只能遍历数组

//不同点:
1.forEach没有返回值, map有返回值是一个数组
```
...
```js
//https://blog.csdn.net/Sunshinedada/article/details/89521968

//相同点:
1.宽度和高度计算是一样的,都是margin+padding+border+width
//不同点:
1.ie中content的宽度包括padding和border


//chrome是标准规范。即width和height不包含padding和border. 
//解决方法:
加上 doctype 声明, 使页面以W3C盒子模型渲染。
...
#### 标准盒子模型(盒子模型范围包括margin,border,padding,content). content部分不包括其他部分.

![标准盒子模型](https://img-blog.csdnimg.cn/2019042518043792.jpg?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1N1bnNoaW5lZGFkYQ==,size_16,color_FFFFFF,t_70)

#### IE盒子(盒子模型范围包括margin,border,padding,content). content部分包含了border+padding)

![IE盒子](https://img-blog.csdnimg.cn/20190425181449528.jpg?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1N1bnNoaW5lZGFkYQ==,size_16,color_FFFFFF,t_70)
...
计算1-n的累和(递归或者reduce): 实现一个求和函数sum(n)==>返回1-n的和
```
...
https://juejin.cn/post/6876060646484492295
...
function serial(tasks){
    let result = [];
    return tasks.reduce((pre,item,index)=>{
        return pre.then(res=>{
...
或 https://jishuin.proginn.com/p/763bfbd37953
...
或 https://segmentfault.com/a/1190000016832285

async function runPromiseByQueue(myPromise){
    for(let value of myPromise){
        await value();
    }
}
```
